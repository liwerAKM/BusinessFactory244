...
{  /// <summary>
   ...
   /// </summary>
    public abstract class DbHelperMySQL
    {
        ...
        public static string ConnectionStringMySQL
        {
            get
            {
                if (string.IsNullOrEmpty(_connectionString))
                    _connectionString = MyPubConstant.ConnectionStringEBPP;// ConfigurationManager.AppSettings["ConnectionStringEBPP"];

                //try
                //{
                //    if (ConfigurationManager.AppSettings["ConStringEncrypt"].ToLower() == "true")
                //    {
                //        _connectionString = AESHelper.DecryptDefault(_connectionString);
                //        Console.WriteLine($"数据参数解密:{_connectionString}");
                //    }
                //    else
                //    {
                //        string test = AESHelper.EncryptDefault(_connectionString);
                //        Console.WriteLine($"数据参数加密:{test}");
                //    }
                //}
                //catch (Exception ex)
                //{
                //    Console.WriteLine($"数据参数解密失败:{ex.Message }");
                //}
        ...
        public static DataSet Query(string SQLString)
        {
            using (MySqlConnection connection = new MySqlConnection(ConnectionStringMySQL))
            {
                ...
                catch (MySql.Data.MySqlClient.MySqlException ex)
                {
                    ...
                    Console.WriteLine(string.Format("Error MySqlException:from {0} \r\n", ex.Message));
                    return null;
                }
                return ds;
            }
        ...
        /// <param name="SQLString">SQL语句</param>
        /// <returns>影响的记录数</returns>
        public static int ExecuteSqlTran(string SQLString, params MySqlParameter[] cmdParms)
        {
            using (MySqlConnection connection = new MySqlConnection(ConnectionStringMySQL))
            {
                connection.Open();
        ...
        /// <returns></returns>
        public static bool GetSysIdBase(string SYSID_NAME, out Int64 Sys_Id)
        {
            Sys_Id = -1;
            using (MySqlConnection conn = new MySqlConnection(ConnectionStringMySQL))
            {
                ...
                MySqlParameter sql_param = sqlcmd.Parameters.Add("SYSID", MySql.Data.MySqlClient.MySqlDbType.Int64);
                ...
                else if ((Int64)sql_param.Value <= 0)
                {
                    return false;
                }
                else
                {
                    Sys_Id = (Int64)sql_param.Value;
                    return true;
                }
            }
        }
        ...
        public static bool GetBusIdBase(string BusID_NAME, out int Sys_Id)
        {
            ...
            using (MySqlConnection conn = new MySqlConnection(ConnectionStringMySQL))
            {
                ...
                MySqlParameter sql_param = sqlcmd.Parameters.Add("SYSID", MySqlDbType.Int64);
                sql_param.Direction = ParameterDirection.Output;
                sqlcmd.ExecuteNonQuery();
                if (sql_param.Value == DBNull.Value)
                {
