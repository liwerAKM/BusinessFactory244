using CommonModel;
using DB.Core;
using OnlineBusHos968_Common.Model;
using Soft.Core;
using System;
...
namespace OnlineBusHos968_Common.BUS
{
    class GETPATRECORD
    {
        ...
        public static string DoBusiness(string json_in)
        {
            ...
            try
            {
                ...
                if (dtpat_info.Rows.Count == 0)
                {
                    ...
                    pat_info.BIRTHDAY = ageAndUnit.BirthDay.ToString("yyyy-MM-dd");
        ...
        public static string UnDoBusiness(string json_in)
        {
            ...
            try
            {
                GETPATRECORD_M.GETPATRECORD_IN _in = JSONSerializer.Deserialize<GETPATRECORD_M.GETPATRECORD_IN>(json_in);
                GETPATRECORD_M.GETPATRECORD_OUT _out = new GETPATRECORD_M.GETPATRECORD_OUT();
                XmlDocument doc = QHXmlMode.GetBaseXml("SENDCARDINFO", "1");
                ...
                XMLHelper.X_XmlInsertNode(doc, "ROOT/BODY", "BIRTHDAY", string.IsNullOrEmpty(_in.BIR_DATE) ? "" : _in.BIR_DATE.Trim());
