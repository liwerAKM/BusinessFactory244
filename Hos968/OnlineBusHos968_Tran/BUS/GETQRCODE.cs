...
using System.Xml;
using System.Data;
using OnlineBusHos968_Tran.Model;
using Com.Alipay.Domain;
using Com.Alipay.Business;
using Alipay.AopSdk.Core.Response;

namespace OnlineBusHos968_Tran.BUS
{
    class GETQRCODE
    {
            ...
            try
            {
                ...
                else if (GlobalVar.DoBussiness == "1")
                {
                    ...
                    if (GlobalVar.Linux == "0")
                    {
                        ...
                        {
                            OnlineBusHos968_Tran.F2FPAY.Windows.WxPayTradePrecreateBuilder builder = new OnlineBusHos968_Tran.F2FPAY.Windows.WxPayTradePrecreateBuilder();
                            builder.out_trade_no = out_trade_no;
                            ...
                            OnlineBusHos968_Tran.F2FPAY.Windows.WxPayTradePrecreateBuilder F2FPayResult = InDataRe.GetBusinessData<OnlineBusHos968_Tran.F2FPAY.Windows.WxPayTradePrecreateBuilder>(Key);
                        ...
                        else 
                        {
                            ...
                            string TID = "";
                            //获取终端号对应信息
                            DataTable dt = new Plat.MySQLDAL.BaseFunction().GetList_ZZJ("baccountposc2btid", "HOS_ID='"+_in.HOS_ID+"' AND LTERMINAL_SN='" + _in.LTERMINAL_SN + "'", "tid");
                            if (dt.Rows.Count > 0) 
                            {
                                TID = dt.Rows[0]["tid"].ToString().Trim();
                            }
                            posin.TID = TID;
                    ...
                    else //if (GlobalVar.Linux == "1")
                    {
                        ...
                        else if(_in.DEAL_TYPE== "1")//微信
                        {
                            ...
                            F2FPAY.Linux.WxPayTradePrecreateBuilder F2FPayResult = InDataRe.GetBusinessData<OnlineBusHos968_Tran.F2FPAY.Linux.WxPayTradePrecreateBuilder>(Key);
                            _out.HIS_RTNXML = F2FPayResult.DateRe;
                            if (F2FPayResult.result_code.ToString() == "SUCCESS")
                            {
                                #region 保存wechat_tran表数据
