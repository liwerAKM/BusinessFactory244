...
namespace OnlineBusHos968_Tran
{
    class GlobalVar
    {
        ...
        /// <param name="subSubId"></param>
        /// <returns></returns>
        public static bool CallOtherBus(string data, string busid, string subSubId, out string outdata)
        {
            ...
            try
            {
                ...
                SLBBOtherBus.BusID = busid;
                SLBBOtherBus.SubBusID = subSubId;
                SLBBOtherBus.BusData = data;
                SLBBusinessInfo OutSLBBOtherBus;

                result = BusServiceAdapter.Ipb_CallOtherBusiness(SLBBOtherBus, out OutSLBBOtherBus);
                if (!result)
                {
                    outdata = "Call[" + SLBBOtherBus.BusID + "] Fail";
                    result= false;
                    goto EndPoint;
                }

                if (OutSLBBOtherBus.ReslutCode == 1)
                {
                    outdata = OutSLBBOtherBus.BusData;
                    result= true;
                    goto EndPoint;
                }
                else
                {
                    outdata = OutSLBBOtherBus.ResultMessage;
                    result = false;
                    goto EndPoint;
                }
        ...
        {
            string Key = EncryptionKeyCore.KeyData.AESKEY(Hos_id);
            var http = new Soft.Core.HttpClient(F2FPAY_URL + "PBusF2FHelper");
            InteractiveData InData = new InteractiveData();
            InData.ServiceName = ServiceName;
            InData.SetBusinessData(IndataT, Key);
