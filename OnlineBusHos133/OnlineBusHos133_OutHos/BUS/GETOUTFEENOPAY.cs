    ...
    class GETOUTFEENOPAY
    {
        public static string B_GETOUTFEENOPAY(string json_in)
        {
            ...
            try
            {
                ...
                if (string.IsNullOrEmpty(_in.HOSPATIDS))//20220922增加字段
                {
                    dataReturn.Code = 0;
                    dataReturn.Msg = "HOSPATIDS不能为空";
                    dataReturn.Param = JSONSerializer.Serialize(_out);
                    goto EndPoint;
                }

                foreach (string HOSPATID in _in.HOSPATIDS.Split('|'))
                {
                    ...
                    try
                    {
                        ...
                        if(_out.PRELIST ==null || _out.PRELIST.Count == 0)
                        {
                            dataReturn.Code = 7;
                            dataReturn.Msg = "未查询到处方信息";
                            goto EndPoint;
                        }
