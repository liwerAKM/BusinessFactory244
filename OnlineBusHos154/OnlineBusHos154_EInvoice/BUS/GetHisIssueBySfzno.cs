...
using OnlineBusHos154_EInvoice.Model;
using Soft.Core;
using Newtonsoft.Json.Linq;

namespace OnlineBusHos154_EInvoice.BUS
{
    class GetHisIssueBySfzno
    {
        public static string B_GetHisIssueBySfzno(string json_in)
        {
            ...
            try
            {
                ...
                dic.Add("Bizcode", "");//发票号
                dic.Add("patient_id", FormatHelper.GetStr(_in.YLCARD_NO));//院内卡号或门诊号
                //dic.Add("Bizcode", Bizcode);
                //dic.Add("patient_id", patient_id);
                dic.Add("begin_date", FormatHelper.GetStr(_in.BEGIN_DATE) == "" ? DateTime.Now.AddDays(-360).ToString("yyyy-MM-dd") : FormatHelper.GetStr(_in.BEGIN_DATE));
