...
namespace OnlineBusHos172_Report
{
    class GlobalVar
    {
        ...
        public static string GetConfig(string configname)
        {
            ...
            docini.Load(Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "bin", "OnlineBusHos172_Report.dll.config"));
        ...
        private static byte[] SoapWebServiceEncodeParsToSoap_LIS(Hashtable pars, string methodName)
        {
            //string str = "<?xml version=\"1.0\" encoding=\"utf-8\"?>";
            StringBuilder strInner = new StringBuilder();
            foreach (DictionaryEntry de in pars)
            {
                strInner.Append($"<lis:{de.Key}><![CDATA[{de.Value}]]></lis:{de.Key}>");
            }
            string inner = $@"<soapenv:Envelope xmlns:soapenv=""http://schemas.xmlsoap.org/soap/envelope/"" xmlns:lis=""http://LisQueryService.org/"">
               <soapenv:Header/>
               <soapenv:Body>
                  <lis:{methodName}>
                  {strInner}
            ...
            return Encoding.UTF8.GetBytes(inner);
        }


        public static bool CALLSERVICE_LIS(string inxml, Hashtable hashtable,string MethodName, ref string his_rtnxml)
        {
            DateTime intime = DateTime.Now;
            try
            {
                string url = "http://192.16.39.17:8009/LisQueryService.asmx";
                ...
                var data = SoapWebServiceEncodeParsToSoap_LIS(hashtable, MethodName);
