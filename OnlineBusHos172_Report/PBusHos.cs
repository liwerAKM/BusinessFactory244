...
{
    class PBusHos172_Report : ProcessingBusinessAsyncResult
    {
            ...
            try
            {
                ...
                switch (name)//CCN.ToString().Substring(CCN.ToString().Length - 4)
                {
                    case "0001"://获取患者检验报告列表
                        OutBusinessInfo.BusData = BUS.GETLISREPORT.A_GETLISREPORT(InBusinessInfo.BusData);
                        break;
                    case "0002":// 查询检验报告明细
                        OutBusinessInfo.BusData = BUS.GETLISRESULT.A_GETLISRESULT(InBusinessInfo.BusData);
                        break;
                    case "0003":// 检验报告打印回传
                        OutBusinessInfo.BusData = BUS.ZZJLISPRNBACK.A_ZZJLISPRNBACK(InBusinessInfo.BusData);
                        break;
