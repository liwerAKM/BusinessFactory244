...
namespace OnlineBusHos172_Tran
{
    public class PubFunc
    {
        ...
        public static InteractiveData F2FPay_201<T>(string Hos_id, T IndataT, string ServiceName)
        {
            ...
            string url = @"http://192.16.34.57:8017/service.asmx";
            //SqlSugarModel.SysConfig model = db.Queryable<SqlSugarModel.SysConfig>().Where(t => t.HOS_ID == Hos_id && t.config_key == "F2FPAY_URL").First();
            //if (model != null)
            //{
            //    url = model.config_value;
            //}
            XmlDocument doc = WebServiceHelper.QuerySoapWebService(url, "F2Fpay", hashtable);
        ...
        public static InteractiveData F2FPay<T>(string Hos_id, T IndataT, string ServiceName)
        {
            ...
            hashtable.Add("strbuilder", InData.BuildJson());
            var db = new DbMySQLZZJ().Client;
            string url = @"http://yun.ztejsapp.cn:9002/AliPay/WebServiceF2F.asmx";
            SqlSugarModel.SysConfig model = db.Queryable<SqlSugarModel.SysConfig>().Where(t => t.HOS_ID == Hos_id && t.config_key == "F2FPAY_URL").First();
            if (model != null)
            {
                url = model.config_value;
