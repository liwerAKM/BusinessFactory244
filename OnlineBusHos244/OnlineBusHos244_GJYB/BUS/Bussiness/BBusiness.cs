...
using OnlineBusHos244_GJYB.Model;
using Soft.Core;
using System;
...
namespace OnlineBusHos244_GJYB.BUS
{
    ...
    public class BBusiness : IBusiness
    {
        public virtual DataReturn PSNQUERY(string injson)
        {
            ...
            bool flag2 = CSBHelper.CreateInputRoot(HOS_ID, "1101", "",opter_no, insuplc_admdvs, t1101, ref inputRoot1101, ref msg);
            if (!flag)
            {
                dataReturn.Code = 1;
                dataReturn.Msg = msg;
            ...
            chsPsn.chsInput1101 = JsonConvert.SerializeObject(inputRoot1101);
        ...
        public virtual DataReturn PSNQUERYTEST(string injson)
        {
            DataReturn dataReturn = new DataReturn();
            GJYB_PSNQUERY_M.GJYB_PSNQUERY_IN _in = JSONSerializer.Deserialize<GJYB_PSNQUERY_M.GJYB_PSNQUERY_IN>(injson);
            GJYB_PSNQUERY_M.GJYB_PSNQUERY_OUT _out = new GJYB_PSNQUERY_M.GJYB_PSNQUERY_OUT();
            ...
            //ref 需要初始化
            Models.InputRoot inputRoot1101 = new Models.InputRoot();
            //用Models.T1101.Root 而不是直接用data 能让入参的json外面多套一层data，直接用data 就没data节点了
            flag = CSBHelper.CreateInputRoot(HOS_ID, infno, "", opter_no, insuplc_admdvs, t1101, ref inputRoot1101, ref msg);//inputroot1101即医保返回
            if (!flag)//这尼玛拼参数拼不了还调个毛
            {
                dataReturn.Code = 1;
                dataReturn.Msg = msg;
                return dataReturn;
            }
            //调医保需要的参数，globalvar来搞定即可，给我hosid ，我来查hospital
            Models.OutputRoot outputRoot1101 = GlobalVar.YBTrans(HOS_ID, inputRoot1101);
            if (outputRoot1101.infcode != "0")//调医保能调错，直接 datareturn掉，后面完不成了
            {
                dataReturn.Code = 1;
                dataReturn.Msg = outputRoot1101.err_msg;//乱起八糟一堆属性
                return dataReturn;
            }

            Models.RT1101.Root rt1101 = outputRoot1101.GetOutput<Models.RT1101.Root>();

            //.root就能多一个root类 的内部类名的节点
            Models.T5360.Root input5360 = new Models.T5360.Root();
            input5360.data = new Models.T5360.data();//类的类对象初始化
            input5360.data.begntime = DateTime.Now.ToString("yyyy-MM-dd");
            input5360.data.insutype = "";
            input5360.data.psn_name = rt1101.baseinfo.psn_name;
            input5360.data.psn_no = rt1101.baseinfo.psn_no;
        ...
        public virtual DataReturn REGTRY(string injson)
        {
            ...
            PBusHos244_GJYB.WriteLog("查询人员医保信息", "出参",chspsn.psn_no);
            string chsOutput1101 = chspsn.chsOutput1101;
            string chsInput1101 = chspsn.chsInput1101;
            ...
            XMLHelper.X_XmlInsertNode_NOCHANGE(doc, "ROOT/BODY", "chsInput1101", chsInput1101);
            ...
            string his_rtnxml = "";
            PBusHos244_GJYB.WriteLog("调用his入参", doc.InnerXml, chspsn.psn_no);
            ...
            try
            {
                ...
                else
                {
                    ...
                    PBusHos244_GJYB.WriteLog("调用his接口成功", "出参", his_rtnxml);
            ...
            JObject jin2201 = JObject.Parse(chsInput2201);//看他样子像json 就用这种方法转josn ，JObject jin2201 =JObject.Parse(chsInput2201)
