    ...
    class GETSCHPERIOD
    {
        public static string B_GETSCHPERIOD(string json_in)
        {
            ...
            try
            {
                ...
                //his_rtnxml = "< ROOT >< HEADER >< TYPE > GETSCHPERIOD </ TYPE >< CZLX > 0 </ CZLX ></ HEADER >< BODY >< CLBZ > 0 </ CLBZ >< CLJG > 执行成功 </ CLJG >< PERIODLIST >< PERIOD >< PERIOD_START > 08 : 00:00 </ PERIOD_START >< PERIOD_END > 11:30:00 </ PERIOD_END >< COUNT_REM > 200 </ COUNT_REM ></ PERIOD ></ PERIODLIST ></ BODY ></ ROOT >".Replace(" ", "");
                ...
                try
                {
                    ...
                    try
                    {
                        ...
                        if (string.IsNullOrEmpty(_in.DOC_NO))//普通号
                        {
                            try
                            {
                                dtDept = XMLHelper.X_GetXmlData(xmldoc, "ROOT/BODY/PERIODLIST").Tables[0];
                            ...
                            COUNT_REM = dtDept.Rows[0]["COUNT_REM"].ToString().Trim();
                        ...
                        else//专家号
                        {
                            try
                            {
                                dtDoc = XMLHelper.X_GetXmlData(xmldoc, "ROOT/BODY/PERIODLIST").Tables[0];
                            }
                            catch
                            {
                                dataReturn.Code = 5;
                        ...
                        foreach (DataRow dr in dtDept.Rows)
                        {
                            ...
                            period.PERIOD_START = dtDept.Columns.Contains("PERIOD_START") ? dr["PERIOD_START"].ToString() : "";
                            period.PERIOD_END = dtDept.Columns.Contains("PERIOD_END") ? dr["PERIOD_END"].ToString() : "";
                            period.COUNT_REM = dtDept.Columns.Contains("COUNT_REM") ? dr["COUNT_REM"].ToString() : "";
                            //period.REGISTER_TYPE = dtperiod.Columns.Contains("REGISTERCODE") ? dr["REGISTERCODE"].ToString() : "";
                            period.REGISTER_TYPE = dtDept.Columns.Contains("REGISTER_TYPE") ? dr["REGISTER_TYPE"].ToString() : "";
