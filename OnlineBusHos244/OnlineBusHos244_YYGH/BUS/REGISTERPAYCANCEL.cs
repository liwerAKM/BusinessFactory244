    ...
    class REGISTERPAYCANCEL
    {
        public static string B_REGISTERPAYCANCEL(string json_in)
        {
            ...
            try
            {
                ...
                XMLHelper.X_XmlInsertNode(doc, "ROOT/BODY", "YNCARDNO", string.IsNullOrEmpty(_in.HOSPATID) ? "" : _in.HOSPATID.Trim());
                XMLHelper.X_XmlInsertNode(doc, "ROOT/BODY", "DJ_ID", string.IsNullOrEmpty(_in.HOS_SN) ? "" : _in.HOS_SN.Trim());
