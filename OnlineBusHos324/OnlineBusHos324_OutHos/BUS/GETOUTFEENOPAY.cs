        ...
        public static string B_GETOUTFEENOPAY(string json_in)
        {
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(_in.MB_ID))
                {
                    ...
                    XMLHelper.X_XmlInsertNode(doc1, "ROOT/BODY", "YLCARD_TYPE", string.IsNullOrEmpty(_in.YLCARD_TYPE) ? "" : _in.YLCARD_TYPE.Trim());
                ...
                try
                {
                    ...
                    try 
                    {
                        ...
                        //DataTable dt_of_HOS_SN_REG = dv.ToTable(true, "HOS_SN_REG");//挂号流水号

                        //foreach(DataRow dr in dt_of_HOS_SN_REG.Rows)
                        //{
                        //    dv.RowFilter = string.Format(@"HOS_SN_REG='{0}'",dr["HOS_SN_REG"]);
