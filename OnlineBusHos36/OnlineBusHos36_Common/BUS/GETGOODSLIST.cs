...
namespace OnlineBusHos36_Common.BUS
{
    class GETGOODSLIST
    {
        public static string B_GETGOODSLIST(string json_in)
        {
            ...
            try
            {
                ...
                DataSet dsGOODSLIST = XMLHelper.X_GetXmlData(xmldoc, "ROOT/BODY/ITEMLIST");
                if (dsGOODSLIST == null || dsGOODSLIST.Tables.Count == 0)
                {
                    dataReturn.Code = 5;
                    dataReturn.Msg = "未查到对应信息，请确认输入首写拼音";
                    dataReturn.Param = JSONSerializer.Serialize(_out);
                    goto EndPoint;
                }
                DataTable dtGOODSLIST = dsGOODSLIST.Tables[0];
                ...
                if (dtGOODSLIST == null || dtGOODSLIST.Rows.Count == 0)//再差
                {
                    dataReturn.Code = 5;
                    dataReturn.Msg = "未查到对应信息，请确认输入首写拼音";
                    dataReturn.Param = JSONSerializer.Serialize(_out);
