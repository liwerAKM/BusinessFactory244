        ...
        public static string B_TICKETREPRINT(string json_in)
        {
            ...
            try
            {
                ...
                else if (_in.TYPE == "2") 
                {
                    ...
                    var list=  db.Queryable<SqlSugarModel.Ticketreprint>().Where(t => t.HOS_ID == _in.HOS_ID && t.SFZ_NO == _in.SFZ_NO &&t.BIZ_TYPE==_in.PT_TYPE&& SqlFunc.Between(t.NOW, DateTime.Now.AddDays(-1 * TICKETREPRINTDAYS), DateTime.Now)).ToList();
