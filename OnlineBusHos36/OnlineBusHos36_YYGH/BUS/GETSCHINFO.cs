                ...
                try
                {
                    ...
                    if (dtDept != null && dtDept.Rows.Count > 0)
                    {
                        foreach (DataRow dr in dtDept.Rows)
                        {
                            ...
                            dept.SCH_ORDER = dept.SCH_TIME == "上午" ? 1 : (dept.SCH_TIME == "下午" ? 2 : 3);//新增字段用于排序
                    ...
                    if (dtDoc != null && dtDoc.Rows.Count > 0)
                    {
                        foreach (DataRow dr in dtDoc.Rows)
                        {
                            ...
                            doctor.SCH_ORDER = doctor.SCH_TIME == "上午" ? 1 : (doctor.SCH_TIME == "下午" ? 2 : 3);//新增字段用于排序
                    ...
                    if (_in.SCH_TYPE == "02")//当日排班按上下午进行排序
                    {
                        _out.DEPTLIST= _out.DEPTLIST.OrderBy(x => x.SCH_ORDER).ToList();
                        _out.DOCLIST= _out.DOCLIST.OrderBy(x => x.SCH_ORDER).ToList();
                    }
