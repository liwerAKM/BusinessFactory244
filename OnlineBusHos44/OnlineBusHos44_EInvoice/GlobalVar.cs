...
using System.Xml;
using static OnlineBusHos44_EInvoice.BUS.GetHisIssueBySfzno;

namespace OnlineBusHos44_EInvoice
{
    class GlobalVar
    {
        ...
        public static string GetConfig(string configname)
        {
            ...
            docini.Load(Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "bin", "OnlineBusHos44_EInvoice.dll.config"));
        ...
        public static bool CALLSERVICE2(string HOS_ID, string inxml, ref string his_rtnxml)
        {
            DateTime intime = DateTime.Now;
            try
            {
                ...
                if (use_encryption == "1")
                {
                    string secretkey = EncryptionKeyCore.KeyData.AESKEY(HOS_ID);
                    his_rtnxml = AESExample.AESDecrypt(his_rtnxml, secretkey);
                    WriteLog("elecinvoice", "解密出参", his_rtnxml);
                }
                if (DoBussiness == "1")
                {
                    ModLogHos modLogHos = new ModLogHos();
