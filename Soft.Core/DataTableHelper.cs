...
namespace Soft.Core
{
    ...
    public abstract class DataTableHelper
    {
        ...
        static public DataTable GetTable(DataRow[] drs)
        {
            if (drs != null)
            {
                DataTable newTable = drs[0].Table.Clone();
                foreach (DataRow dr in drs)
                {
                    object[] arr = dr.ItemArray;
                    DataRow newrow = newTable.NewRow();
        ...
        static public DataTable GetTable(DataRow dr)
        {
            if (dr != null)
            {
                DataTable newTable = dr.Table.Clone();
