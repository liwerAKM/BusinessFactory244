...
{
    class ZZJ_EInvoiceAPI_b : ProcessingBusinessAsyncResult
    {
        public override bool ProcessingBusiness(SLBBusinessInfo InBusinessInfo, out SLBBusinessInfo OutBusinessInfo)
        {
            ...
            WriteLog("ZZJ_EInvoiceAPIHead", "ReceiveData", JSONSerializer.Serialize(InBusinessInfo));
            try
            {
                string ss = "0001";
                ...
                {
                    case "0001"://获取电子票据列表 
                        OutBusinessInfo.BusData = BUS.GetHisIssueBySfzno_B.B_GetHisIssueBySfzno(InBusinessInfo.BusData);
                        break;
                    case "0002":// 电子票据文件下载
                        OutBusinessInfo.BusData = BUS.ZZJ_EInvoiceDownload_B.B_EInvoiceDownload(InBusinessInfo.BusData);
                        break;
                    case "0003"://电子票据打印回传
                        OutBusinessInfo.BusData = BUS.UpdatePrintStatus_B.B_UpdatePrintStatus(InBusinessInfo.BusData);
                        break;
                    default:
