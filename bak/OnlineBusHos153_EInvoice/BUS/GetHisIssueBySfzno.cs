...
using OnlineBusHos153_EInvoice.Model;
using Soft.Core;
using Newtonsoft.Json.Linq;

namespace OnlineBusHos153_EInvoice.BUS
{
    class GetHisIssueBySfzno
    {
        public static string B_GetHisIssueBySfzno(string json_in)
        {
            ...
            try
            {
                ...
                dic.Add("begin_date", FormatHelper.GetStr(_in.BEGIN_DATE));
                dic.Add("end_date", FormatHelper.GetStr(_in.END_DATE));
                root.BODY = dic;
                string injson = JSONSerializer.Serialize(root);
                string his_rtnxml = "";

                if (!GlobalVar.CallService_Core(_in.HOS_ID, injson, ref his_rtnxml))
                {
                    dataReturn.Code = 1;
