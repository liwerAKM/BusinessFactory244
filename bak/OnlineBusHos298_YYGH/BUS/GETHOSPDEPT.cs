...
namespace OnlineBusHos298_YYGH.BUS
{
    class GETHOSPDEPT
    {
        ...
        public static string DoBusiness(string json_in)
        {
            ...
            try
            {
                GETHOSPDEPT_M.GETHOSPDEPT_IN _in = JSONSerializer.Deserialize<GETHOSPDEPT_M.GETHOSPDEPT_IN>(json_in);
                GETHOSPDEPT_M.GETHOSPDEPT_OUT _out = new GETHOSPDEPT_M.GETHOSPDEPT_OUT();
                int PAGE_COUNT = 0;
                DataTable dtdept_info = new Plat.MySQLDAL.dept_info().GetDeptInfo_ZZJ(_in.USE_TYPE, _in.FILT_TYPE, _in.FILT_VALUE, _in.HOS_ID, _in.RETURN_TYPE, FormatHelper.GetInt(_in.PAGEINDEX)==0?1: FormatHelper.GetInt(_in.PAGEINDEX), FormatHelper.GetInt(_in.PAGESIZE) == 0 ? 1 : FormatHelper.GetInt(_in.PAGESIZE), ref PAGE_COUNT);
                if (dtdept_info.Rows.Count == 0)
                {
                    dataReturn.Code = 1;
                    dataReturn.Msg = "无对应科室信息";
                    goto EndPoint;
                }
                try
                {
                    _out.DEPTLIST = new List<GETHOSPDEPT_M.DEPT>();
                    foreach (DataRow dr in dtdept_info.Rows)
                    {
                        GETHOSPDEPT_M.DEPT dept = new GETHOSPDEPT_M.DEPT();
                        dept.DEPT_CODE = dtdept_info.Columns.Contains("DEPT_CODE") ? dr["DEPT_CODE"].ToString() : "";
                        dept.DEPT_NAME = dtdept_info.Columns.Contains("DEPT_NAME") ? dr["DEPT_NAME"].ToString() : "";
                        dept.DEPT_INTRO = dtdept_info.Columns.Contains("DEPT_INTRO") ? dr["DEPT_INTRO"].ToString() : "";
                        dept.DEPT_URL = dtdept_info.Columns.Contains("DEPT_URL") ? dr["DEPT_URL"].ToString() : "";
                        dept.DEPT_ORDER = dtdept_info.Columns.Contains("DEPT_ORDER") ? dr["DEPT_ORDER"].ToString() : "";
                        dept.DEPT_TYPE = dtdept_info.Columns.Contains("DEPT_TYPE") ? dr["DEPT_TYPE"].ToString() : "";
                        dept.DEPT_ADDRESS = dtdept_info.Columns.Contains("DEPT_ADDRESS") ? dr["DEPT_ADDRESS"].ToString() : "";
                ...
                try
                {
                    ...
                    try
                    {
                        ...
                        foreach (DataRow dr in dtdept.Rows)
                        {
                            ...
                            dept.DEPT_SUP_CODE = dtdept.Columns.Contains("DEPT_SUP_CODE") ? dr["DEPT_SUP_CODE"].ToString() : "";
                            dept.OPERA_TYPE= dtdept.Columns.Contains("OPERA_TYPE") ? dr["OPERA_TYPE"].ToString() : "";
                            dept.REGFLAG= dtdept.Columns.Contains("REGFLAG") ? dr["REGFLAG"].ToString() : "";
                            _out.DEPTLIST.Add(dept);
                        }
                    }
                    catch
