            ...
            try
            {
                ...
                else if (GlobalVar.DoBussiness == "1")
                {
                    ...
                    try
                    {
                        ...
                        _out.BED_NO = dtrev.Columns.Contains("BED_NO") ? dtrev.Rows[0]["BED_NO"].ToString() : "";
                        #region 业务数据保存
                        DataTable dtPAT = new Plat.MySQLDAL.BaseFunction().GetList("pat_info", "sfz_no='" + dtrev.Rows[0]["sfz_no"].ToString().Trim() + "'", "PAT_ID");
                        int pat_id;
                        if (dtPAT.Rows.Count == 0)
                        {
                            ...
                            new Plat.MySQLDAL.pat_info().Add(pat_info);
