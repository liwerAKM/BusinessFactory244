...
namespace ZZJTran
{
    ...
    /// 测试两数相加
    /// </summary>
    public class ZZJTranAPI : ProcessingBusinessAsyncResultByte
    {
        public override bool ProcessingBusiness(int CCN, SLBInfoHeadBusS sLBInfoHeadBusS, byte[] In, out byte[] Out)
        {
            ...
            try
            {
                WriteLog("ZZJTranHead", "ReceiveData", JSONSerializer.Serialize(sLBInfoHeadBusS));
                string InBusinessInfo = base.GetStrData(In);
                InBusinessInfo = System.Web.HttpUtility.UrlDecode(InBusinessInfo);
                WriteLog("ZZJTranAPI", "ReceiveData", InBusinessInfo);
            ...
            WriteLog("ZZJTranAPI", "outData", OutBusinessInfo);
