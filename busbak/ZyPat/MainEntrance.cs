...
namespace ZyPat
{
    ...
    /// 测试两数相加
    /// </summary>
    public class ZyPatAPI: ProcessingBusinessAsyncResultByte
    {
        public override bool ProcessingBusiness(int CCN, SLBInfoHeadBusS sLBInfoHeadBusS, byte[] In, out byte[] Out)
        {
            ...
            try
            {
                WriteLog("ZyPatHead", "ReceiveData", JSONSerializer.Serialize(sLBInfoHeadBusS));
                string InBusinessInfo = base.GetStrData(In);
                InBusinessInfo = System.Web.HttpUtility.UrlDecode(InBusinessInfo);
                WriteLog("ZyPatAPI", "ReceiveData", InBusinessInfo);
            ...
            WriteLog("ZyPatAPI", "outData", OutBusinessInfo);
